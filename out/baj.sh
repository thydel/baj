...
bajl:self () { jq "$jq" "$@"; }
bajl:tst () { < ${1:?}; }
bajl:txt () { loop tst "$@" && cat "$@" | txt1; }
bajl:txt1 () { header; asjs | bm4 | dist | emit; }
bajl:nss () { loop nss1 "$@"; }
...
bajl:exp () { loop exp1 "$@"; }
bajl:lib1 () { nsp ${1:?} || fail not a ns; alias $1; alias $(aliases $1); fn $1; }
