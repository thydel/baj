...
baj:mk-alias yml2sh bajl ''
baj:mk-alias zsrc bajl ''
bajl:opts () { # Use local -A opts; local args cont; opts "$@" in caller
...
bajl:md2js () { local -A jq=('header' '[{ m4: { ME: "\($ns)" }}, { id, ns: "ME" }] + input' 'md2js' 'def txt: .c[2] | map(.c // " ") | join("");
def f($b):
...
.blocks | reduce .[] as $b ([]; f($b)) | map(values)'); pandoc -t json | jq "${jq[md2js]}" | jq "${jq[header]}" -n --arg ns ${1:?}; }
bajl:fail () { unset -v fail; : "${fail:?${FUNCNAME[1]} $@}"; }
bajl:loop () { local i; for i in "${@:2}"; do $1 "$i"; done; }
bajl:list () { loop echo "$@"; }
bajl:args () { mapfile -t; ((${#MAPFILE[@]} > 0)) && "${@:-echo}" "${MAPFILE[@]}"; }
bajl:map () { while read; do "${@:-echo}" "$REPLY"; done; }
bajl:self () { jq "$jq" "$@"; }
bajl:tst () { < ${1:?}; }
bajl:txt () { loop tst "$@" && loop txt1 "$@"; }
bajl:txt1 () { header; < ${1:?} asjs | bm4 | dist | emit; }
bajl:nss () { loop nss1 "$@"; }
bajl:nss1 () { local jq='.[].ns // empty'; < ${1:?} yq -oj | self -r; }
bajl:listid () { loop listid1 "$@"; }
bajl:listid1 () { compgen -c ${1:?}:; compgen -c ns:${1:?}; }
bajl:ids () { loop ids1 "$@"; }
bajl:ids1 () { local jq='.[].id'; eval ${1:?} src | jq "$jq" -r; }
bajl:fn () { loop fn1 "$@"; }
bajl:fn1 () { listid1 ${1:?} | args declare -f; }
bajl:fns () { ids "$@" | args def; }
bajl:def () { loop def1 "$@"; }
bajl:def1 () { [[ -v BASH_ALIASES[${1:?}] ]] || fail $1 not an alias && declare -f ${BASH_ALIASES[$1]}; }
bajl:ns () { (($# == 0)) || fail too many args; compgen -c ns: | cut -d: -f2; }
bajl:nsp () { compgen -c ns:${1:?} > /dev/null; }
bajl:aliases () { loop aliases1 "$@"; }
bajl:aliases1 () { local jq='$ARGS.positional | [.[:$n], .[$n:]] | transpose[] | select(last | test("\($ns):")) | first'; nsp ${1:?} || fail not a ns; self -nr --arg ns $1 --argjson n ${#BASH_ALIASES[@]} --args "${!BASH_ALIASES[@]}" "${BASH_ALIASES[@]}"; }
bajl:forget () { loop forget1 "$@"; }
bajl:forget1 () { . <(${FUNCNAME%%:*}:listid1 ${1:?} | args echo unset -f; nsp $1 && { aliases $1 | args echo unalias; echo unalias $1; }); }
bajl:load () { forget $(nss "$@"); . <(${FUNCNAME%%:*}:txt "$@"); }
bajl:warn () { echo \# generated by ${FUNCNAME[1]}, avoid edit; }
bajl:optal () { echo shopt -s expand_aliases; }
bajl:prudent () { echo set -euo pipefail; echo shopt -s inherit_errexit; }
bajl:exp1 () { listid ${1:?} | args echo export -f; }
bajl:exp () { loop exp1 "$@"; }
bajl:lib1 () { nsp ${1:?} || fail not a ns; alias $1; alias $(aliases $1); . <(zsrc $1); fn $1; }
bajl:lib () { optal; loop lib1 "$@"; def self; exp "$@"; }
bajl:with-lib () { (($#)) || fail; optal; until [[ $# == 0 || $1 == -- ]]; do lib1 $1; shift; done; [[ $1 == -- ]] && shift; def self; (($#)) && { f=${BASH_ALIASES[$1]:-$1}; shift; echo $f "${@@Q}"; }; }
bajl:as-lib () { warn; lib "$@"; }
bajl:as-cmd () { as-lib "$@"; echo 'eval "$@"'; }
bajl:yml2sh () { load ${1:?}; nss1 $1 | args as-cmd; }
bajl:zsrc () { local jq='"\($ns):src () { <<< '\''\(.)'\'' base64 -d | zcat | jq; }"'; ${1:?}:src | jq -c | gzip | base64 | self --arg ns $1 -sRr; }
alias null=ns:null; ns:null () { null:${1:?} "${@:2}"; }
