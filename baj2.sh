...
bajl:forget () { local ns='bajl'; local id='forget'; . <($ns:listid1 ${1:?} | args echo unset -f; nsp $1 && { aliases $1 | args echo unalias; echo unalias $1; }); }
bajl:load () { local ns='bajl'; local id='load'; forget $(nss "$@"); . <($ns:txt "$@"); }
alias bajl=ns:bajl; ns:bajl () { bajl:${1:?} "${@:2}"; }
bajl:src () { <<< '[{"ns":"bajl","id":"opts","sh":"# Use local -A opts; local args cont; opts \"$@\" in caller\nlocal opt val tmp i; args=()\nuntil [[ $# -eq 0 ]]; do\n    case \"$1\" in\n        --) shift; break;;\n        --*=*) opt=\"${1%%=*}\"; val=\"${1#*=}\"; opts[\"${opt:2}\"]=\"$val\"; shift;;\n        --*) opts[\"${1:2}\"]=\"1\"; shift;;\n        -[0-9]*) args+=(\"$1\"); shift;;\n        -) args+=(\"$1\"); shift;;\n        -*) tmp=${1:1}\n            for ((i = 0; i < ${#tmp}; i++ )); do\n                opt=\"${tmp:$i:1}\"\n                opts[$opt]=$((opts[$opt] + 1))\n            done\n            shift;;\n        *) args+=(\"$1\"); shift;;\n    esac\ndone; cont=(\"$@\")"},{"ns":"bajl","id":"fail","sh":"unset -v fail; : \"${fail:?${FUNCNAME[1]} $@}\""},{"is":["fa"],"ns":"bajl","id":"loop","sh":"local i; for i in \"${@:2}\"; do $1 \"$i\"; done"},{"ns":"bajl","id":"list","sh":"loop echo \"$@\""},{"is":["fa"],"ns":"bajl","id":"args","sh":"mapfile -t; ((${#MAPFILE[@]} > 0)) && \"${@:-echo}\" \"${MAPFILE[@]}\""},{"is":["fa"],"ns":"bajl","id":"map","sh":"while read; do \"${@:-echo}\" \"$REPLY\"; done"},{"ns":"bajl","id":"self","sh":"jq \"$jq\" \"$@\""},{"ns":"bajl","id":"tst","sh":"< ${1:?}"},{"ns":"bajl","id":"txt1","sh":"header; asjs | bm4 | dist | emit"},{"ns":"bajl","id":"txt","sh":"loop $ns:tst \"$@\" && cat \"$@\" | txt1"},{"ns":"bajl","id":"nss1","jq":".[].ns // empty","sh":"< ${1:?} yq -oj | self -r"},{"ns":"bajl","id":"nss","sh":"loop nss1 \"$@\""},{"ns":"bajl","id":"listid1","sh":"compgen -c ${1:?}:; compgen -c ns:${1:?}"},{"ns":"bajl","id":"listid","sh":"loop listid1 \"$@\""},{"ns":"bajl","id":"ids1","jq":".[].id","sh":"eval ${1:?} src | jq \"$jq\" -r"},{"ns":"bajl","id":"ids","sh":"loop ids1 \"$@\""},{"ns":"bajl","id":"fn1","sh":"listid1 ${1:?} | args declare -f"},{"ns":"bajl","id":"fn","sh":"loop fn1 \"$@\""},{"ns":"bajl","id":"fns","sh":"ids \"$@\" | args def"},{"ns":"bajl","id":"def1","sh":"[[ -v BASH_ALIASES[${1:?}] ]] || fail $1 not an alias && declare -f ${BASH_ALIASES[$1]}"},{"ns":"bajl","id":"def","sh":"loop def1 \"$@\""},{"ns":"bajl","id":"nsp","sh":"compgen -c ns:${1:?} > /dev/null"},{"ns":"bajl","id":"aliases","sh":"loop aliases1 \"$@\""},{"ns":"bajl","id":"aliases1","jq":"$ARGS.positional | [.[:$n], .[$n:]] | transpose[] | select(last | test(\"\\($ns):\")) | first","sh":"nsp ${1:?} || fail not a ns; self -nr --arg ns $1 --argjson n ${#BASH_ALIASES[@]} --args \"${!BASH_ALIASES[@]}\" \"${BASH_ALIASES[@]}\""},{"ns":"bajl","id":"forget","sh":". <($ns:listid1 ${1:?} | args echo unset -f; nsp $1 && { aliases $1 | args echo unalias; echo unalias $1; })"},{"ns":"bajl","id":"load","sh":"forget $(nss \"$@\"); . <($ns:txt \"$@\")"}]' jq; }
